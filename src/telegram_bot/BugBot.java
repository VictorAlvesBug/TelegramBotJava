...
public class BugBot {
	...
	// Classe que gerenciará os comandos disponíveis e suas respectivas ações
	private GerenciadorComandosBot gerenciadorComandos;

	// Construtor recebe as chaves de acesso via parâmetro para fazer a configuração
	// inicial do bot
	public BugBot(String tokenTelegramBot, String apiKeyMovies) throws Exception {
		...
		// Define quais são os comandos disponíveis no bot em cada contexto
		gerenciadorComandos = new GerenciadorComandosBot(apiKeyMovies);
	}

	// Método executado em loop, para verificar se há alguma mensagem enviada pelo
	// usuário
	...
	private void tratarMensagemRecebida(Update mensagem) {
		...
		// Armazena apenas o comando recebido, desconsiderando espaços e quebras de
		// linha no início e no final
	...
	private void responderComando(Long chatId, String comando) {
		...
		String strListaRespostas = retornarRespostas(comando);

		// Envia resposta
		boolean enviou = bot.execute(new SendMessage(chatId, strListaRespostas)).isOk();

		// Exibe no console se a resposta foi enviada com sucesso ou não
		printarStatusMensagemEnviada(enviou, strListaRespostas);
	}
	...
	private String retornarRespostas(String strComandos) {
		...
		// Converte a lista de comandos recebidos para o seguinte formato:
		// [
		// "/comandoSemParametro",
		...
		// Caso nenhum comando seja reconhecido, aborta a execução, informando que os
		// comandos não foram encontrados
		...
		for (String comandoRecebido : listaComandosRecebidos) {
			...
			if (indicePrimeiroSeparador == -1) {
				...
				// Caso o primeiro separador exista, separa o primeiro trecho (comando) do resto
				// (parâmetros)
		...
		// Flag que sinalizará se pelo menos um comando executado alterou a pilha de
		// comandos pai.
		boolean algumComandoAlterouPilha = false;

		// Itera em cada conjunto comandoComParametro para verificar se algum deles
		// alterou a pilha de comandos pai.
		for (String comandoRecebido : listaComandosRecebidos) {

			// Recupera o comando sem nenhum parâmetro (mesmo que o comando tenha parâmetro,
			...
			// Caso o comando sem parâmetro seja o "/inicio" ou seja o comando pai mais
			// próximo na pilha, é entendido que a execução do comando alterou a pilha,
			// marcando a flag como true.
			if (gerenciadorComandos.verificarSeComandoAlterouPilha(comandoSemParametros)) {
				algumComandoAlterouPilha = true;
			}
		}

		// Caso tenha algum comando que tenha alterado a pilha, a lista de comandos
		// possíveis, deve ter sido alterada, então exibe a lista de comandos
		// atualizada.
		if (algumComandoAlterouPilha) {
			return gerenciadorComandos.retornarStrListaComandos(sbRespostas.toString());
		}

		// Retorna um compilado das respostas dos comandos
		return sbRespostas.toString();
	}

	private List<String> retornarListaComandosRecebidos(String strComandos) {
		// Contador de aspas para saber se o caracter " " (espaço) é um separador de
		// trechos ou apenas um caracter de texto
		int contadorAspas = 0;

		// Buffer responsável por armazenar cada caracter até que o trecho esteja
		// completo
		...
		// Ex3 (depois): ["/comandoComDoisParametros", "parametro1", "Parâmetro com
		// espaço"]
		...
		for (char caracter : strComandos.toCharArray()) {
			...
			// Espaço: caso não esteja dentro de aspas, finaliza o trecho atual e usa o " "
			// (espaço) como separador de trechos.
			case ' ':
				// Entende-se que o conjunto de caracteres que está entre uma aspa ímpar e uma
				// par é apenas texto.
				// Desta forma, se o número de aspas encontradas até então for par, significa
				// que o " " (espaço) encontrado é realmente um separador de trechos.
				// Porém, se o número de aspas encontradas for impar, significa que o " "
				// (espaço) encontrado é parte de um texto e não deve ser considerado como um
				// separador de trechos.
				if (contadorAspas % 2 == 0) {
					// " " é um separador de trechos, então adiciona o trecho na lista e limpa a
					// variável
		...
		// O que adicionou os trecho na lista até o penúltimo foi o " " (espaço) que
		// veio logo depois.
		// Como o último trecho não possui um " " (espaço) depois, ele deve ser
		// adicionado separadamente.
		if (sbTrecho.toString().length() > 0) {
			listaTrechos.add(sbTrecho.toString());
		}

		// Buffer que guarda cada caracter do conjunto comando-parâmetro antes de
		// adicionar na lista.
		...
		// Ex3 (antes): ["/comandoComDoisParametros", "parametro1", "Parâmetro com
		// espaço"]
		...
		for (String trecho : listaTrechos) {
			// Remove qualquer ";", pois este caracter será utilizado como separação entre
			// os trechos
			trecho = trecho.replace(";", "");

			// Verifica se o trecho é reconhecido como um comando válido
			if (gerenciadorComandos.verificarSeComandoExiste(trecho)) {
				// Caso tenha algo concatenado no comandoComParametro, adiciona este "algo" na
				// lista.
				// Este "algo" pode ser um comando simples ou um comando com parâmetros.
				// Mas como o "trecho" é considerado como um comando, corta o trecho anterior e
				// o adiciona na lista
				...
				// Se o comandoComParametro não está preenchido significa que o primeiro trecho
				// não é um comando válido.
				// Desta forma, aborta a execução do método, pois o primeiro trecho precisava
				// ser um comando válido.
				if (sbComandoComParametro.toString().isEmpty()) {
					return null;
				}

				// Caso o comandoComParametro tenha algo dentro, concatena um separador ";" e o
				// "trecho" atual;
		...
		// O que adicionou os trecho na lista até o penúltimo foi o comando válido que veio logo
		// depois.
		// Como o último trecho não possui um comando depois, ele deve ser adicionado
		// separadamente.
		if (sbComandoComParametro.toString().length() > 0) {
			listaComandosComParametros.add(sbComandoComParametro.toString());
		}
	...
	private void printarStatusMensagemEnviada(boolean enviou, String mensagem) {
		...
		// Caso contrário, informa erro ao enviar mensagem
